...
PopUpSel.prototype = {
    ...
    init: function () {
        ...
        $this.$el.on('click', '.ui-columns-foot-ok', function () {
            ...
            $('.table thead tr th', $this.$el).each(function () {
    ...
    setUI: function () {
        ...
        var tbl = '<div class="' + ($this.Data.length > 0 ? 'ui-columns-table-data' : 'ui-columns-table-nodata') + '" style="overflow:auto;">' +
            '<table class="table table-hover" style="border-bottom:1px solid #ccc;"><thead><tr>';
                    ...
                    if ($this.SortBy == col) {
                        if ($this.InReverse) {
                            tbl += '<th class="' + 'ui-table-sort-down' + '" data-columns-sortby="' + col + '">' +
                                col.replace(re47, '/').replace(re, ' ') + ' <span class="bi bi-sort-down"></span></th>';
                        } else {
                            tbl += '<th class="' + 'ui-table-sort-up' + '" data-columns-sortby="' + col + '">' +
                                col.replace(re47, '/').replace(re, ' ') + ' <span class="bi bi-sort-down-alt"></span></th>';
                        }
                    } else {
        ...
        var foot =
            '<div class="ui-table-footer">' +
                '<div class="ui-table-results" style="margin:5px 20px 0 0;float:left">' +
                    '<div>' +
                        'Page <strong>' + $this.CurPage + '</strong> of <strong>' + $this.PageCount + '</strong>' +
                    '</div>' +
                '</div>' +
                '<div class="ui-table-show-rows" style="float:left">' +
                    '<span>' +
                        'Rows:&nbsp;' +
                        '<select class="form-select-sm-popup">' + opts + '</select>' +
                    '</span>' +
                '</div>' +
                '<div class="ui-table-controls" style="white-space:nowrap;">' +
                    '<div class="input-group">' +
                        '<div class="' + (navFirst ? 'ui-table-control-first btn-outline-secondary' : 'ui-table-control-disabled btn-outline-secondary') + '" data-columns-page="' + navFirst + '">' +
                            '<span aria-hidden="true">&laquo;</span>' +
                        '</div>' +
                        '<div class="' + (navPrev ? 'ui-table-control-prev btn-outline-secondary' : 'ui-table-control-disabled btn-outline-secondary') + '" data-columns-page="' + navPrev + '">' +
                            '<span aria-hidden="true">&lt;</span>' +
                        '</div>' +
                        '<span>' +
                            '<input class="ui-table-control-gotopage" type="text" name="page" />' +
                        '</span>' +
                        '<div class="' + (navNext ? "ui-table-control-next btn-outline-secondary" : "ui-table-control-disabled btn-outline-secondary") + '" data-columns-page="' + navNext + '">' +
                            '<span aria-hidden="true">&gt;</span>' +
                        '</div>' +
                        '<div class="' + (navLast ? "ui-table-control-last btn-outline-secondary" : "ui-table-control-disabled btn-outline-secondary") + '" data-columns-page="' + navLast + '">' +
        ...
        $('.table tbody tr', $this.$el).click(function (event) {
        ...
        $('.table tbody tr', $this.$el).dblclick(function (event) {
        ...
        $.each(hdnInd, function (idx, val) {
            ...
            $('.table tr > *:nth-child(' + (parseInt(val) + 1) + ')', $this.$el).hide();
        });
