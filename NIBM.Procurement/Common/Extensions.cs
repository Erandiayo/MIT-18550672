...
namespace System
{
    public static class SysExtensions
    {
        ...
        public static T CopyContent<T>(this T src, T res, string propsToIncl = null, string propsToExcl = null, bool skipReferences = false)
        {
            ...
            foreach (var prop in src.GetType().GetProperties())
            {
                ...
                if (other != null && prop.PropertyType == other.PropertyType && other.CanWrite)
                {
                    ...
                    try { other.SetValue(res, prop.GetValue(src, null), null); }
                    catch { }
                }
            }
            return res;
        ...
        public static T DeProxyClone<T>(this T src, int diveInLength = 2, bool createBaseType = true) where T : class
        {
            if (src == null)
                return default(T);
            ...
            foreach (var prop in retTyp.GetProperties())
            {
                ...
                try
                {
                        ...
                        !propType.IsEnum)
                    {
                        ...
                        if (val != null && val.GetType().FullName.StartsWith("Castle.Proxies."))
                        {
                            if (!createBaseType)
                                continue;

                            var proxyBase = Activator.CreateInstance(val.GetType().BaseType);
                            val.CopyContent(proxyBase, skipReferences: true);
                            val = proxyBase;
                        }
                        ...
                        if (prop.PropertyType.GetInterface("IEnumerable") != null)
                        {
                            ...
                            {
                                tempCollection.Add(obj.DeProxyClone(diveInLength - 1, createBaseType));
                            }
                            val = tempCollection;
                        }
                        else if (prop.PropertyType.IsClass)
                        {
                            ...
                            val = val.DeProxyClone(diveInLength - 1, createBaseType);
                        }
                    }
        ...
        public static void SetObject<T>(this HttpSessionStateBase session, T src) where T : class
        {
            var lazyLoadingEnabled = false;
            dbNIBMContext dbContext = null;
            try
            {
                var ctrl = HttpContext.Current.Items[BaseController.cikCurrentController] as BaseController;

                if (ctrl != null)
                {
                    dbContext = ctrl.GetDBContext();
                    lazyLoadingEnabled = dbContext.ChangeTracker.LazyLoadingEnabled;
                    dbContext.ChangeTracker.LazyLoadingEnabled = false;
                }

                session[BaseController.sskCrtdObj] = src.DeProxyClone();
            }
            finally
            {
                if (dbContext != null)
        ...
        public static void SetObject<T>(this HttpSessionStateBase session, IEnumerable<T> src) where T : class
        {
            var lst = new List<T>();
            foreach (var item in src)
            {
                lst.Add(item.DeProxyClone());
            }

            session[BaseController.sskCrtdObj] = lst;
        }
        public static List<T> GetObjectList<T>(this HttpSessionStateBase session) where T : class
        {
            var obj = session[BaseController.sskCrtdObj];

            if (obj == null)
                return null;

            if (!obj.GetType().Equals(typeof(IEnumerable<T>)))
                return null;
        ...
        public static T GetObject<T>(this HttpSessionStateBase session) where T : class
        {
            var obj = session[BaseController.sskCrtdObj];
