...
namespace System.Web
{
    public static class WebHelpers
    {
        public static Tentity GetEntity<Tentity, Tmodel>(this IModel<Tentity, Tmodel> mapper)
        {
            ...
            using (var db = new dbNIBMContext())
            {
        ...
        public static void CopyData(object source, object target, int level = 2, Type type = null)
        {
            if (--level == 0)
                return;
            ...
            foreach (var prop in source.GetType().GetProperties())
            {
                ...
                PropertyInfo other = target.GetType().GetProperty(prop.Name);
                var val = prop.GetValue(source, null);

                if (IsAssignable(val, other))
                {
                    if (val.GetType().Namespace == "Castle.Proxies")
                    {
                        CopyData(val, other.GetValue(target), level, other.PropertyType);
                    }
                    else
                        other.SetValue(target, val);
                }
            }
        }


        public static void LoadReference<T, TEntity, TModel>(this IModel<TEntity, TModel> obj, Expression<Func<TEntity, T>> expr) where T : class
        {
            using (var dbctx = new dbNIBMContext())
            {
                ...
                var entBase = dbctx.Model.GetEntityTypes().Where(x => x.ClrType.Name == entType.Name).FirstOrDefault();
                var navProps = entBase.GetPropertiesAndNavigations().Where(x => x is Navigation).ToList();

                var navProp = (Navigation)navProps.Where(x => x.Name == (expr.Body as MemberExpression).Member.Name).FirstOrDefault();

                var depProps = navProp.ForeignKey.Properties;
                object[] keyVals = depProps.Select(x => entType.GetProperty(x.Name).GetValue(ent)).ToArray();

                var refEnt = dbctx.Set<T>().Find(keyVals);
...
namespace System.Web.Mvc.Html
{
    public static class MvcHtmlHelpers
    {
        ...
        public static MvcHtmlString DetailsViewFor<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, string dataFormat = null, object htmlAttributes = null)
        {
            var pi = GetPropertyInfo(expression);
            ...
            if (isMulti)
            {
                ...
                t.InnerHtml = txt;
                t.Attributes.Add("class", "form-control form-control-sm");
            ...
            else
            {
                ...
                t.Attributes.Add("class", "form-control form-control-sm rounded-pill");
                t.Attributes.Add("readonly", "readonly");
            }

            object attrs = htmlAttributes.ToDynamic();
            foreach (KeyValuePair<string, object> kvp in (ExpandoObject)attrs)
            { t.Attributes.Add(kvp.Key, kvp.Value.ToString()); }
            ...
            string emptyText = "--Select--", string dspFormat = "{0} - {1}", string valueMember = null, int popUpWidth = 500, string hiddenIndices = null)
        {
            ...
            s.Attributes.Add("data-title", "<span class=\"bi bi-info-circle\" style=\"font-size:medium;color:blue\"></span> " + title);
        ...
        public static MvcHtmlString Image(this HtmlHelper htmlHelper, string action, string controller = null, object routeValues = null, object htmlAttributes = null)
        {
            UrlHelper urlHelper = new UrlHelper(htmlHelper.ViewContext.RequestContext);

            TagBuilder builder = new TagBuilder("img");
            builder.Attributes["src"] = controller == null ? urlHelper.Action(action, routeValues) : urlHelper.Action(action, controller, routeValues);
            builder.MergeAttributes(new RouteValueDictionary(HtmlHelper.AnonymousObjectToHtmlAttributes(htmlAttributes)));

            return MvcHtmlString.Create(builder.ToString());
        }

        public static MvcHtmlString NoEncodeLabel(this HtmlHelper htmlHelper, string text, string title, object htmlAttributes = null)
        {
            UrlHelper urlHelper = new UrlHelper(htmlHelper.ViewContext.RequestContext);
        ...
        public static MvcHtmlString ExtEditorFor<TModel, TProperty>(this HtmlHelper<TModel> htmlHelper, Expression<Func<TModel, TProperty>> expression, bool isEditable, object additionalViewData)
        {
            ...
            else
            {
                ...
                foreach (KeyValuePair<string, object> kvp in (ExpandoObject)attrs)
                { newAttrs.Add(kvp.Key, kvp.Key != "class" ? kvp.Value : kvp.Value.ToString().Replace("form-control form-control-sm rounded-pill", "")); }
